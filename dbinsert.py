...
def bolum_ekle():
    try:
        ...
        df = pd.read_excel("Bolum_listesi.xlsx")

        for index, row in df.iterrows():
            program_adi = str(row["ProgramAdi"])
            bolum_sayisi = str(row["BolumNo"])
            uzunluk = str(row["Uzunluk"])
            ...
            cursor.execute("""
                INSERT INTO Bolum (ProgramID, BolumNo, Uzunluk)
                VALUES (?, ?, ?)
                """, (program_id, bolum_sayisi, uzunluk))
